...
spec = do
    describe "LibHslicer.PlanarSlice.sliceContours" $ do
       it "Slices a Mesh with 0.2 layer height" $
          sliceContours B.boxMesh sParamsDefault `shouldBe` boxSlices
    ...
    describe "LibHslicer.PlanarSlice.printPrep" $ do
       it "Prepares contour for printing" $
         printPrep [B.contour1] sParamsDefault `shouldBe` [B.contour1Combination] 
       it "Prepares bigger contour for printing" $
         printPrep [map (mapV (*2)) B.contour1] sParamsDefault `shouldBe` [B.contour1_x2Combination]
    
    describe "LibHslicer.PlanarSlice.toGcmd" $ do
       it "Creates G-Code from sliced contours" $ do
         writeGCode "./contour1.gcode" $ toGCmd $ printPrep [map (mapV (*100)) B.contour1] sParamsDefault