...
spec = do
    ...
    describe "LibHslicer.PlanarSlice.calcPParams" $ do
       it "Calculates print parameters for adjacent vertices" $
          calcPParams B.v1 B.v2 sParamsDefault `shouldBe` PParams {_extMove = ((sqrt 5 * 0.4 * 0.2)/(pi * (1.75 / 2) ^ 2), "mm2"), _velocity = _speed sParamsDefault}
       it "Calculates print parameters for same vertex" $
          calcPParams B.v1 B.v1 sParamsDefault `shouldBe` PParams {_extMove = (0.0, "mm2"), _velocity = _speed sParamsDefault}
